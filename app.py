...
def parse_employee_file(file_obj):
    """Returns role_map and location_map from an uploaded Employee.xlsx."""
...
def parse_project_file(file_obj):
    """Returns client_map from an uploaded Project.xlsx."""
    df = pd.read_excel(file_obj, header=None)
    client_map = {}
    for i, row in df.iterrows():
        vals = [str(v).strip() if pd.notna(v) else '' for v in row]
...
st.markdown("Upload your files below and generate the consolidated report instantly.")
st.divider()

# ── Section 1: Mapping files ──────────────────────────────────────────────────
st.subheader("Step 1 — Upload mapping files")
st.caption("Upload these when your employee list or project list changes. Otherwise skip — the previously uploaded version will be used for this session.")

col1, col2 = st.columns(2)
with col1:
    emp_file = st.file_uploader("Employee.xlsx", type=["xlsx"], key="emp",
                                 help="Columns: Employee Name | Role | Location")
with col2:
    proj_file = st.file_uploader("Project.xlsx", type=["xlsx"], key="proj",
                                  help="Columns: Project Name | Client")

# Parse mapping files — use uploaded file if provided, else fall back to session state
if emp_file:
    role_map, location_map = parse_employee_file(emp_file)
    st.session_state['role_map']     = role_map
    st.session_state['location_map'] = location_map
    st.success(f"✅ Employee.xlsx loaded — {len(role_map)} employees")
elif 'role_map' in st.session_state:
    role_map     = st.session_state['role_map']
    location_map = st.session_state['location_map']
    st.info(f"ℹ️ Using previously uploaded Employee.xlsx — {len(role_map)} employees")
else:
    role_map = {}
    location_map = {}
    st.warning("⚠️ Employee.xlsx not uploaded — Role and Location will be blank")

if proj_file:
    client_map = parse_project_file(proj_file)
    st.session_state['client_map'] = client_map
    st.success(f"✅ Project.xlsx loaded — {len(client_map)} projects")
elif 'client_map' in st.session_state:
    client_map = st.session_state['client_map']
    st.info(f"ℹ️ Using previously uploaded Project.xlsx — {len(client_map)} projects")
else:
    client_map = {}
    st.warning("⚠️ Project.xlsx not uploaded — Client will be blank")

st.divider()

# ── Section 2: MIS Report ─────────────────────────────────────────────────────
st.subheader("Step 2 — Upload this week's MIS Report")

mis_file = st.file_uploader("MIS Report (.xlsx)", type=["xlsx"], key="mis",
                              help="The weekly export from your MIS system.")

if mis_file:
    st.info(f"📄 **{mis_file.name}**")
        ...
        with st.spinner("Processing..."):
            try:
                ...
                if not output_rows:
                    ...
                    base        = os.path.splitext(mis_file.name)[0]
